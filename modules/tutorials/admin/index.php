...
function SaveTutorial()
{
    ...
    if (0 == $tid) {
        ...
        $xoopsDB->query('INSERT INTO ' . $xoopsDB->prefix('tutorials') . " (tid, cid, gid, tname, tdesc, timg, tcont, tlink, tauthor, status, codes, hits, rating, votes, date, submitter, dir, timgwidth, timgheight) VALUES ($newid, $cid, $gid, '$tname', '$tdesc', '$timg', '$tcont', '$tlink', '$tauthor', $status, $codes, 0, 0, 0, $time, $submitter, $dir, $timgwidth, $timgheight)") or $eh::show('0013');
    } elseif (0 == $status) {
        $xoopsDB->query('UPDATE ' . $xoopsDB->prefix('tutorials') . " set tid=$tid, cid=$cid, gid=$gid, tname='$tname', tdesc='$tdesc', timg='$timg', tcont='$tcont', tlink='$tlink', tauthor='$tauthor', status=$status, codes=$codes, hits=0, rating=0, votes=0, date=$time, timgwidth=$timgwidth, timgheight=$timgheight where tid=$tid") or $eh::show('0013');
        ...
        $xoopsDB->query('UPDATE ' . $xoopsDB->prefix('tutorials') . " set tid=$tid, cid=$cid, gid=$gid, tname='$tname', tdesc='$tdesc', timg='$timg', tcont='$tcont', tlink='$tlink', tauthor='$tauthor', status=$status, codes=$codes, date=$date, timgwidth=$timgwidth, timgheight=$timgheight where tid=$tid") or $eh::show('0013');
    } else {
        $date = time();
