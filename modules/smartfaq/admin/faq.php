...
switch ($op) {
    ...
    $offline = $_POST['offline'] ?? 1;
    if ((_SF_STATUS_OFFLINE == $faqObj->status()) && (0 == $offline)) {
    ...
    if ($confirm) {
        ...
        xoops_cp_header();
        xoops_confirm(['op' => 'del', 'faqid' => $faqObj->faqid(), 'confirm' => 1, 'name' => $faqObj->question()], 'faq.php', _AM_SF_DELETETHISARTICLE . " <br>'" . $faqObj->question() . "'. <br> <br>", _AM_SF_DELETE);
        xoops_cp_footer();
